	...
	public partial class XCProject : System.IDisposable
	{
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false, string compilerFlags = null )
		{
			...
			if (!string.IsNullOrEmpty(compilerFlags)) {
				Debug.Log("AddFile " + filePath + ", " + parent + ", " + tree + ", " + (createBuildFiles? "TRUE":"FALSE") + ", " + (weak? "TRUE":"FALSE") + "," + " compilerFlags:" + compilerFlags ); 
			}
			...
			fileReference = new PBXFileReference( filePath, (TreeEnum)System.Enum.Parse( typeof(TreeEnum), tree ), compilerFlags );
			...
			if( !string.IsNullOrEmpty( fileReference.buildPhase ) && createBuildFiles ) {
				...
				switch( fileReference.buildPhase ) {
					case "PBXFrameworksBuildPhase":
						...
						BuildAddFile(fileReference,currentObject,weak, filePath);
						}
					...
					case "PBXSourcesBuildPhase":
						...
						Debug.LogFormat( "Adding Source Build File. fileRef={0},name = {1}, compiler flag:{2}",fileReference.ToCSV(),fileReference.name, fileReference.compilerFlags );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
		...
		public void AddEmbedFramework( string fileName)
		{
			...
			//create a build file
			PBXBuildFile buildFile = new PBXBuildFile( fileReference,false,fileName );
		...
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXFrameworksBuildPhase> currentObject,bool weak,string filePath = null)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak, filePath );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		}
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXResourcesBuildPhase> currentObject,bool weak,string filePath = null)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak, filePath );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		}
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXShellScriptBuildPhase> currentObject,bool weak,string filePath = null)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak,filePath );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		}
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXSourcesBuildPhase> currentObject,bool weak,string filePath = null)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak, filePath );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		}
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXCopyFilesBuildPhase> currentObject,bool weak,string filePath = null)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak, filePath );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		...
		{	
			...
			foreach( string filePath in mod.files ) {
				...
				if (file.IndexOf(':') >= 0) {
					compilerFlags = file.Substring(file.IndexOf(':') + 1 );
					file = file.Substring(0,file.IndexOf(':'));
				}

				string absoluteFilePath = System.IO.Path.Combine( mod.path, file );
			...
			foreach( string folderPath in mod.folders ) {
				...
				if (path.IndexOf(':') >= 0) {
					compilerFlags = path.Substring(path.IndexOf(':') + 1 );
					path = path.Substring(0,path.IndexOf(':'));
				}


				string absoluteFolderPath = System.IO.Path.Combine( mod.path, path );
				Debug.Log ("Adding folder " + absoluteFolderPath);
				this.AddFolder( absoluteFolderPath, modGroup, (string[])mod.excludes.ToArray( typeof(string) ),true,true,compilerFlags );
			}
