...
{
    [AddComponentMenu("UI/Scroll Rect Base", 37)]
    [SelectionBase]
    [ExecuteInEditMode]
    [DisallowMultipleComponent]
    [RequireComponent(typeof(RectTransform))]
	public abstract class ScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
		//==========ScrollRectBase==========
		...
		private int totalCount;  //negative means INFINITE mode
//		[HideInInspector]
		...
		private List<IData> m_datas = new List<IData> ();
		public void SetData<T>(List<T> datas, int startFillIndex = 0) where T:IData
		{
			DestoryChildren ();
			...
			this.itemTypeStart = this.itemTypeEnd = startFillIndex;
			this.m_datas.Clear ();
			for (int i = 0; i < datas.Count; i++) {
				this.m_datas.Add (datas [i]);
			}
//			ResetRenderData ();
		...
		protected void ResetRenderData()
		{
			for(int i = 0;i<content.transform.childCount;i++)
			{
				GameObject go = content.transform.GetChild(i).gameObject;
				if (i < this.m_datas.Count) {
					Libs.UIBase uiBase = go.GetComponent<Libs.UIBase>();
					uiBase.SetData (this.m_datas[i]);
					uiBase.Refresh ();
				} else {
					Destroy (go);
				}
		...
		public void SetStartFirstIndex(int index)
		{
			this.itemTypeStart = this.itemTypeEnd = index;
		}

		protected virtual RectTransform InstantiateNewItem(int itemIdx)
		{
...
//			RectTransform nextItem = null;
			...
			GameObject itemRender = Libs.PoolMgr.DefaultPools.GetOrCreatePool(this.Render.GetInstanceID().ToString()).CreateObject( this.Render) as GameObject;

//			GameObject itemRender = Instantiate (this.Render); 
			itemRender.transform.SetParent (content.transform, false);
			itemRender.name = "itemRender"+itemIdx;
			Libs.UIBase uiBase = itemRender.GetComponent<Libs.UIBase>();
			uiBase.SetData (this.m_datas[itemIdx]);
			uiBase.Refresh ();
			return uiBase.transform as RectTransform;
		}
		...
		protected float NewItemAtStart()
		{
			...
			float size = 0;
			int i = 0;
			while(i < contentConstraintCount)
			{				
				itemTypeStart--;
				RectTransform newItem = this.GetFromUnVisible (itemTypeStart);
				if (newItem == null) {
					newItem = InstantiateNewItem (itemTypeStart);
				...
				ScrollRectRender render = newItem.GetComponent<ScrollRectRender> ();
				if (render != null) {
					render.ChangeData ();
				}
...
//				RectTransform oldItem = content.GetChild(0) as RectTransform;
				RectTransform oldItem = GetNeedToUnvisibleTransform();
				size = Mathf.Max(GetSize(oldItem), size);
//				prefabPool.ReturnObjectToPool(oldItem.gameObject);
				this.AddToUnVisible(itemTypeStart,oldItem.gameObject);
		...
		protected float NewItemAtEnd()
		{
			...
			while( i < contentConstraintCount)
			{
				RectTransform newItem = this.GetFromUnVisible (itemTypeEnd);
				if (newItem == null) {
					newItem = InstantiateNewItem (itemTypeEnd);
//					yield return InstantiateNewItem (itemTypeEnd,delegate(RectTransform obj) {
//						newItem = obj;
//					});
				}
...
//				RectTransform oldItem = content.GetChild(content.childCount - 1) as RectTransform;
				RectTransform oldItem = GetNeedToUnvisibleTransform(true);
				size = Mathf.Max(GetSize(oldItem), size);
//				oldItem.SendMessage("ScrollCellReturn", SendMessageOptions.DontRequireReceiver);
//				prefabPool.ReturnObjectToPool(oldItem.gameObject);
				itemTypeEnd--;
				this.AddToUnVisible(itemTypeEnd,oldItem.gameObject);
		...
		protected Dictionary<int,GameObject> poolItems = new Dictionary<int,GameObject> ();
		private void AddToUnVisible(int index, GameObject go)
		{
			if (this.Render != null) {
					Libs.PoolMgr.DefaultPools.GetOrCreatePool(this.Render.GetInstanceID().ToString()).DestoryObject(go);
			}
			else{
				if (!poolItems.ContainsKey (index)) {
					poolItems.Add (index, go);
				} else {
					poolItems [index] = go;
				}
//				go.transform.SetParent (null);
		...
		private RectTransform GetFromUnVisible(int index)
		{
			RectTransform result =null;
			GameObject resultGameObject = null;
			poolItems.TryGetValue (index, out resultGameObject);
			if (resultGameObject != null) {	
				resultGameObject.SetActive (true);
				resultGameObject.transform.SetParent (content);
				resultGameObject.transform.localScale = new Vector3 (1f, 1f, 1f);
				result = resultGameObject.transform as RectTransform;

				Libs.UIBase uiBase = resultGameObject.GetComponent<Libs.UIBase> ();
				if (uiBase != null) {
					uiBase.Refresh ();
				}
			}
		...
		private RectTransform GetNeedToUnvisibleTransform(bool isRevert = false)
		{
			int startIndex = isRevert ? content.childCount -1: 0;
			int endIndex = isRevert ? 0 : content.childCount-1;
			if (isRevert) {
				for (int i = startIndex; i >= endIndex; i--) {
					RectTransform _rectTransform = content.GetChild (i) as RectTransform;
        ...
        private void UpdateBounds()
        {
			...
			if (Application.isPlaying && totalCount>0 && UpdateItems(m_ViewBounds, m_ContentBounds))
			{
				Canvas.ForceUpdateCanvases();
				m_ContentBounds = GetBounds();
			}
