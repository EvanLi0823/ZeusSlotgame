...
//	public bool isActive = false;
	...
	public void AddMiddlePointRelatively(Vector3 pos) {
		AddMiddlePointRelatively(startPoint, endPoint, pos);
	}

	public void AddMiddlePointRelatively(Vector3 start, Vector3 end, Vector3 pos) {
		Vector3 vec = end - start;
		middlePoints.Add(new Vector3(vec.x * pos.x, vec.y * pos.y, vec.z * pos.z) + start);
	}
	...
	public string PointsLog() {
		...
		for (int i = 0; i < middlePoints.Count; i++) {
			ret += "Mid(" + i.ToString() + "): " + middlePoints[i].ToString() + '\n';
		}
		ret += "End: " + endPoint.ToString();
