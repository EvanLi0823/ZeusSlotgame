...
///Number up text.
/// </summary>
public class CommonNumberUpText : MonoBehaviour {
    ...
    public void SetNumberUpText(string text,bool enableAnimation= true){
        ...
        long target = ConvertToLong (text);
        long pre = ConvertToLong (preTargetText);
    ...
    public virtual void SetAutoNumberUpText(double baseNumber,int stepNumber, bool enableAnimation = true)
    {
        if (TestController.Instance) return;
        if (!hasInited) InitParameterData();
        string baseText = "$"+ Utils.Utilities.ThousandSeparatorNumber(baseNumber);
        string stepText = Utils.Utilities.ThousandSeparatorNumber(stepNumber);
        long stepValue = ConvertToLong(stepText);
        if (enableAnimation)
        {
            if (finished)
            {
                SetStepText(baseText, false);
                currentCo = StartCoroutine(AutoIncreaseNum((long)baseNumber, stepNumber,baseText));
            }
            else
            {
                StopCoroutine(currentCo);//目前由于协程终止的不确定性，导致程序在任意时刻都有可能被终止，所以需要刷新一次。
    ...
    protected virtual IEnumerator AutoIncreaseNum(long baseNumber, long stepNumber,string baseText)
    {
        long from = baseNumber;
        finished = false;
        long firstHalf = stepNumber / 2;
        long secondHalf = (stepNumber % 2 == 1) ? (firstHalf + 1) : firstHalf;
        bool first = true;
    ...
    protected long  winStep = 3;
    protected string preTargetText = "";
    protected virtual IEnumerator IncreaseNum(string text){
        long to = ConvertToLong (text);
        long from = preNumber;
        finished = false;
        while (from<to) {
            yield return new WaitForSeconds (0.5f);
    ...
    protected void SetStepText(string text,bool enableAni= true){
    ...
    protected void InitParameterData(){
        hasInited = true;
    ...
    private void InitNumberBitSequenceData (string text,bool enableAnimation){
        ...
        long currentNumber = ConvertToLong(text);
        long currentTempNumberBit = currentNumber;
        long preTempNumberBit = preNumber;
        int numberbitListSize = numberBitList.Count;
        for (int i = 0; currentTempNumberBit > 0; i++) {
            if (preTempNumberBit == currentTempNumberBit)
                break;
            int idx = i / 3 + i;//LQ 考虑分隔符，逢三进一
            CommonNumberBitControl nBC = numberBitList [numberbitListSize-1-idx].GetComponentInChildren<CommonNumberBitControl> ();
            List<int> number_Bit = new List<int> ();
            int currentBit = (int)(currentTempNumberBit % 10);
            ...
            if (i > 0 && i%3==0) {
                CommonNumberBitControl nBC_Comma = numberBitList [numberbitListSize-(4*(i/3))].GetComponentInChildren<CommonNumberBitControl> ();
                nBC_Comma.SetUnNumberText (',');
            }
        }
        if (currentNumber==0) {
            CommonNumberBitControl nBC = numberBitList [1].GetComponentInChildren<CommonNumberBitControl> ();
            if (enableAnimation) {
                List<int> number_Bit = new List<int> ();
                number_Bit.Add (0);
        ...
        CommonNumberBitControl nBC_Prefix = numberBitList [0].GetComponentInChildren<CommonNumberBitControl> ();
    ...
    private void CheckEnableCharBitAnimation(string text){
        ...
        for (int i = 0; i < temp.Length; i++) {
            animator = numberBitList [i].GetComponentInChildren<Animator> ();
