namespace CardSystem
{
    public class CardSystemConstants
    {
        public static string CardSystem = "CardSystem";
        public static string LotteryConfig = "LotteryConfig";
        public static string EndTime = "EndTime";
        public static string Icons = "Icons";
        public static string Open = "Open";
        public static string CardLottery = "CardLottery";
        public static string CardPack = "CardPack";
        public static string WeightGroup = "WeightGroup";
        public static string SpinGroup = "SpinGroup";
        public static string Card = "Card";
        public static string Weights = "Weights";
        public static string CardInfo = "CardInfo";
        public static string Cards = "Cards";
        public static string SpinLimit = "SpinLimit";

        public static string Coins = "Coins";


        public static string CardReel = "CardReel";
        public static string Condition = "Condition";
        public static string Percent = "Percent";
        public static string AD = "AD";
        public static string CostCoins = "CostCoins";
        public static string cost = "cost";
        public static string limit = "limit";
        public static string type = "type";

        public static string level = "level";

        public static string index = "index";
        public static string WeightIndex = "WeightIndex";
        public static string Name = "name";
        public static string UIConfig = "UIConfig";
        
        public static string GetCardNewTypeCountMsg = "GetCardNewTypeCountMsg";
        public static string RefreshLotteryMsg = "RefreshLotteryMsg";
        public static string GetCardNewCountMsg = "GetCardNewCountMsg";

        public static string PrefabPath = "CardSystem/Prefab";
    }
}