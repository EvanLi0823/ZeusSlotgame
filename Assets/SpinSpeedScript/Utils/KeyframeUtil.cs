...
namespace CurveExtended{
	...
	public class KeyframeUtil {
		...
		public static Keyframe GetNew(float time, float value, TangentMode left, TangentMode right){
			...
			SetKeyTangentMode(boxed, TangentDirection.Left, left);
			SetKeyTangentMode(boxed, TangentDirection.Right, right);
		...
		public static void SetKeyTangentMode(object keyframe, TangentDirection leftRight, TangentMode mode)
		{
		...
		// UnityEditor.CurveUtility.cs (c) Unity Technologies
		public static TangentMode GetKeyTangentMode(int tangentMode, TangentDirection leftRight)
		{
			if (leftRight == TangentDirection.Left)
